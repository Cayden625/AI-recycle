...
const AssistantChat: React.FC<AssistantChatProps> = ({ onBack }) => {
  ...
  // Use a getter for the chat instance to handle late API_KEY injection
  const getChat = () => {
    if (!chatRef.current) {
      ...
      chatRef.current = ai.chats.create({
        ...
        config: {
          systemInstruction: 'You are a helpful, eco-conscious recycling expert. Answer questions about recycling and the environment.',
        },
      });
    }
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are a recycling expert assistant. Keep answers concise.',
          speechConfig: {
  ...
  return (
    <div className="h-full flex flex-col animate-fadeIn overflow-hidden bg-slate-50">
      <div className="flex items-center justify-between p-3 bg-white border-b border-slate-100">
        ...
        <button onClick={() => { setMessages([]); setTranscription(''); }} className="p-2 text-slate-400 hover:text-red-500 transition-colors">
          <Trash2 size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
            <div className={`max-w-[85%] p-4 rounded-2xl font-bold shadow-sm ${msg.role === 'user' ? 'bg-emerald-600 text-white rounded-br-none' : 'bg-white border-2 border-slate-100 text-slate-800 rounded-bl-none'}`}>
              {msg.text}
            </div>
          </div>
        ))}
      ...
      {isActive && (
        <div className="px-4 py-3 bg-emerald-50 border-y border-emerald-100 flex flex-col items-center space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-emerald-700 font-black text-xs uppercase tracking-widest">{isSpeaking ? 'AI Speaking' : 'Listening...'}</span>
          </div>
        </div>
      )}
      ...
      <div className="p-4 bg-white border-t border-slate-100 flex items-center gap-3">
        {!isActive ? (
          <>
            <button onClick={startVoiceSession} className="p-4 bg-slate-100 text-slate-600 rounded-2xl hover:bg-emerald-100 transition-all shadow-sm">
              <Mic size={24} />
            </button>
            <div className="flex-1 relative flex items-center">
              <input
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-4 pl-4 pr-14 font-bold outline-none focus:border-emerald-500"
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSendTextMessage()}
              />
              <button onClick={handleSendTextMessage} className={`absolute right-2 p-2 rounded-xl ${inputText.trim() ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
                {isTyping ? <Loader2 size={20} className="animate-spin" /> : <Send size={20} />}
              </button>
            </div>
          </>
        ) : (
          <button onClick={stopVoiceSession} className="w-full p-5 bg-red-500 text-white rounded-2xl font-black flex items-center justify-center gap-2 shadow-xl">
